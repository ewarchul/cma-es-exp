des_ja = function(par, fn, ..., lower, upper, control = list()) {
...
#' @param x
  ...
  initlambda = controlParam("lambda", 4 * N) 

  #' Population ending size
  minlambda = controlParam("minlambda", 4 * N) 
  ...
  cc = controlParam("ccum", mu / (mu + 2)) 
  
  #' Size of evolution path
  pathLength = controlParam("pathLength", 6) 
  
  #' Evolution Path decay factor
  cp = controlParam("cp", 1 / sqrt(N)) 

  #' Maximum number of iterations after which algorithm stops
  maxiter = controlParam("maxit", floor(budget / (lambda + 1))) 
  
  #' Path Length Control reference value
  pathRatio = controlParam("pathRatio", sqrt(pathLength))   

  #' Size of the window of history - the step length history
  histSize = controlParam("history", ceiling(6 + ceiling(3 * sqrt(N)))) 
 
  #' Escape from flat-land scale factor 
  ...
  Ft_denom = controlParam("Ft_denom", 1/3)
...
#  log.eigen = controlParam("diag.eigen", log.all)
 ...
 # if (log.eigen) eigen.log = matrix(0, nrow = 0, ncol = N)
  ...
  steps = array(0, pathLength * N) 

  dMean = array(0, dim = c(N, histSize))

  #' Array buffer containing 'histSize' last values of 'Ft'
  ...
  while (counteval < budget) {
    ...
    #' Pointer to the history buffer head
    histHead = 0 
  ...
  #    if (log.eigen) eigen.log = rbind(eigen.log, rev(sort(eigen(cov(t(population)))$values)))
      ...
      historySample = sample(1:limit, lambda, TRUE)
      historySample2 = sample(1:limit, lambda, TRUE)
      ...
      eval_xmeanOld <- apply(t(oldMean), 1, function(x) fn(x, ...))

      pop_quart = stats::ecdf(sort(fitness))
      pTarget <- 1/5
      ps <- pop_quart(eval_xmeanOld)
      FtMultExp <- (ps-pTarget)/(0.5 - pTarget)
      Ft <- Ft*exp(Ft_denom * FtMultExp)
