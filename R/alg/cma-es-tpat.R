cma_es_tpat <- function(par, fn, ..., lower, upper, if_CMA = TRUE, control = list()) {
  norm <- function(x) {
  ...
  d_sigma = controlParam("d_sigma", sqrt(N))
  c_sigma = controlParam("c_sigma", 0.3)
  s = controlParam("s", 0)
  ...
  while (counteval < budget) {
    ...
    x_1 =
      xmean + 
        sigma * norm(rnorm(N)) * (xmean - xmean_old) / (norm(xmean - xmean_old))
    x_2 =
      xmean - 
        sigma * norm(rnorm(N)) * (xmean - xmean_old) / (norm(xmean - xmean_old))
    eval_x_1 = 
      apply(t(x_1), 1, function(x) fn(x, ...) * fnscale)
    eval_x_2 = 
      apply(t(x_2), 1, function(x) fn(x, ...) * fnscale)
    ext_ranks =
      c(eval_x_1, eval_x_2, arfitness) %>%
      base::rank()
    s = 
      c_sigma * (ext_ranks[2] - ext_ranks[1]) / (lambda - 1)
  ...
  res <- list(
    ...
    message = msg,
    label = stringr::str_glue("cma-es-tpat-dsigma-{round(d_sigma, 2)}"),
    constr.violations = cviol,
    diagnostic = log
  )
  class(res) <- "cma_es_tpat.result"
  return(res)
}
