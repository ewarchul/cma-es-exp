cma_es_tpa <- function(par, fn, ..., lower, upper, if_CMA = TRUE, control = list()) {
  norm <- function(x) {
  ...
  alpha = controlParam("alpha", 0.5)
  alpha_prim = controlParam("alpha_prim", 0.5)
  alpha_s = controlParam("alpha_s", 0)
  beta = controlParam("beta", 0)
  c_alpha = controlParam("c_alpha", 0.3)
  ...
  while (counteval < budget) {
    ...
    ## TPA rule
    f_minus = 
      apply(t(xmean - alpha_prim * sigma * zmean), 1, function(x) fn(x, ...) * fnscale)
    f_plus =
      apply(t(xmean + alpha_prim * sigma * zmean), 1, function(x) fn(x, ...) * fnscale)
    alpha_act = 
      if (f_minus < f_plus) -alpha + beta else alpha
    alpha_s = 
      (1 - c_alpha) * alpha_s + c_alpha * alpha_act
  ...
  res <- list(
    ...
    message = msg,
    label = stringr::str_glue("cma-es-tpa-alpha-{alpha}"),
    constr.violations = cviol,
    diagnostic = log
  )
  class(res) <- "cma_es_tpa.result"
  return(res)
}
