...
class CMA_ES(ABC):
    ...
    def __init__(
        ...
        self.sc_tolx = kwargs.get("sc_tolx", 10**(-12) * self.sigma_init)
        self.lambda_ = kwargs.get("lambda", 4 * self.N)
        self.maxiter = kwargs.get("maxit", math.floor(self.budget / self.lambda_))
        self.mu = kwargs.get("mu", math.floor(self.lambda_ / 2))
        self.weights = kwargs.get("weights", math.log(self.mu + 1) - np.log(np.arange(1, self.mu + 1)))
        self.weights = self.weights / self.weights.sum()
        self.mueff = kwargs.get("mueff", sum(self.weights)**2 / sum(self.weights ** 2))
        self.cc = kwargs.get("cc", 4 / (self.N + 4))
        self.cs = kwargs.get("cs", (self.mueff + 2) / (self.N + self.mueff +3))
        self.mucov = kwargs.get("mucov", self.mueff)
        self.ccov = kwargs.get("ccov", (1 / self.mucov) * (2/((self.N + 1.4) ** 2)) + (1 - 1/self.mucov) * ((2 * self.mucov - 1) / ((self.N + 2)**2 + 2 *self.mucov)))
        self.damps = kwargs.get("damps", 1 + 2 * max(0, math.sqrt((self.mueff - 1) / (self.N + 1)) - 1) + self.cs)
        self.chiN = math.sqrt(self.N) * (1 - 1 / (4 * self.N) + 1 / (21 * self.N**2))
        self.population = np.random.uniform(self.lower, self.upper, (self.N, self.N))
    ...
    def adapt_matrix(self, C, BDz, pc, hsig):
        ...
        rank_mu = np.matmul(BDz, np.matmul(np.diag(self.weights), BDz.transpose()))
        return (1 - self.ccov) * C + \
